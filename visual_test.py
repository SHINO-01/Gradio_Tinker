...
import datetime
import time
#===bug: the avatar for chatbot flickers before generating answers. 
...
def chatbot_response(user_input, chat_history, selected_context):
    ...
    # ✅ Extract text if user input is a dictionary (from multimodal input)
    ...
    if not user_text:
        ...
        return chat_history, ""  # ✅ Ensure two values are always returned

    # ✅ Get context description
    context_description = RAG_CONTEXTS.get(selected_context, "General Chatbot")

    # ✅ Append user message properly formatted
    updated_history = list(chat_history)
    updated_history.append({"role": "user", "content": user_text})

    # ✅ Step 1: Immediately append a valid placeholder message
    placeholder_message = {"role": "assistant", "content": "🤖 Typing..."}
    updated_history.append(placeholder_message)

    yield updated_history, ""  # ✅ Ensures Gradio never receives an empty message

    # ✅ Step 2: Short delay to allow UI to process
    time.sleep(0.1)  # Reduced delay to prevent long avatar flickering  

    # ✅ Step 3: Generate bot response and properly format it
    bot_reply = f"[{selected_context} Context] {context_description} - You asked: '{user_text}'"

    # ✅ Step 4: Replace placeholder with final response
    updated_history[-1] = {"role": "assistant", "content": bot_reply}  # ✅ Replace instead of append

    yield updated_history, ""  # ✅ Send final response
...
.chatbot .chat-message.bot img {
    width: 40px !important;  /* ✅ Forces avatar to stay small */
    height: 40px !important;
    object-fit: contain;
    border-radius: 50%;
}

/* === Ensure Placeholder Messages Are Always Shown === */
.chatbot .chat-message.bot:empty::after {
    content: "🤖 Generating response...";  /* ✅ Forces non-empty content */
    display: block;
    font-size: 14px;
    color: #aaa;
    font-style: italic;
